...
parameters {
  ...
  vector<multiplier=(ts_model > 0 ? (ets_noncentered ? R_sd : 1) : 1)>[L+T-max_gen-1] R_noise; // additive errors
...
model {
  ...
  I[1:(L+T)] ~ normal(iota, sqrt(iota)); // half normal due to constraint, approximates Poisson
  }
