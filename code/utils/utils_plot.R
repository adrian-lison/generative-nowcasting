...
plot_nowcast_observed <- function(fit_summary,
  ...
  now <- fit_summary[["now"]]
  mindate <-
    now - time_window # e.g. time_window=28 to show the last four weeks
  maxdate <- now
  ...
  plot <- fit_summary[["nowcast"]] %>%
    ...
    {
      if (show_data_horizons) {
        geom_vline(
          xintercept = fit_summary$start_date,
          linetype = "dashed",
          color = "black"
        )
      }
    } +
    {
      if (show_data_horizons) {
        geom_vline(
          xintercept = fit_summary$start_date + fit_summary$D,
          linetype = "dotted",
          color = "black"
        )
      }
    } +
    {
      if (show_weekends) {
        geom_tile(
          data = .getWeekends(mindate, maxdate),
          aes(
            y = 0,
            height = Inf,
            fill = is_weekend
    ...
    scale_fill_manual(values = c("NA", "grey")) +
    {
      if (!is.null(reported_cases)) {
        geom_col(
          data = reported_cases %>% filter(
            event1_date > mindate,
            event1_date < now
          ),
          aes(x = event1_date, y = n),
          position = "identity",
          color = NA,
          fill = "grey"
        )
    ...
    {
      if (!is.null(truth) &&
        "cases_known" %in% names(truth)) {
        geom_line(
          data = truth %>% filter(date > mindate, date < now),
          aes(y = cases_known),
          color = "red"
        )
      }
    ...
    geom_ribbon(
      aes(ymin = nowcast_known.lower, ymax = nowcast_known.upper),
      fill = "#b3d1ff",
      alpha = 0.6
    ) +
    ...
    scale_x_date(
      expand = c(0, 0),
      breaks = .getDateBreaks(mindate, now - 2, breaks_resolution),
      date_labels = "%b %d"
    ) +
    scale_y_continuous(expand = c(0, 0)) +
    theme(legend.position = "none") +
    coord_cartesian(
      ylim = ifelse(rep(is.null(y_lim), 2),
        c(0, max_cases * 1.05),
        y_lim
      ),
      xlim = c(mindate + 1, now)
    ) +
    ggtitle(paste0(
      "Nowcast from ", now,
      ": Only complete cases (no missing dates)"
    ))
...
plot_nowcast_all <- function(fit_summary,
                             ...
                             interval_width = 0.95,
                             reported_cases = NULL,
                             # unused argument, only for compatibility
                             event1_axislabel = "Date",
                             event2_axislabel = "Count") {
  now <- fit_summary[["now"]]
  mindate <-
  ...
  plot <- fit_summary[["nowcast"]] %>%
    ...
    {
      if (show_infections &&
        !is.null(infections_truth)) {
        geom_line(
          data = infections_truth %>% filter(
            date > mindate,
            date <= maxdate - fit_summary[["median_incubation"]]
          ),
          aes(y = infections),
          color = "red"
        )
    ...
    {
      if (!is.null(truth) &&
        "cases_all" %in% names(truth)) {
        geom_line(
          data = truth %>% filter(date > mindate, date <= maxdate),
          aes(y = cases_all),
          color = "red"
        )
      }
    ...
    {
      if (show_infections &&
        "latent" %in% names(fit_summary)) {
        geom_ribbon(
          data = fit_summary[["latent"]] %>% filter(
            date > mindate,
            date <= maxdate - fit_summary[["median_incubation"]]
          ),
          aes(ymin = .lower, ymax = .upper, y = iota),
          fill = "#ffbf80",
          alpha = 0.3
        )
      }
    ...
    geom_ribbon(
      aes(ymin = nowcast_all.lower, ymax = nowcast_all.upper),
      fill = "#b3d1ff",
      alpha = 0.6
    ) +
    geom_line(color = "#0066ff") +
    ...
    coord_cartesian(
      ylim = ifelse(rep(is.null(y_lim), 2),
        c(0, max_cases * 1.05), y_lim
      ),
      xlim = c(mindate + 1, now)
    ) +
    ggtitle(paste0(
      "Nowcast from ", now,
      ": All cases (date estimated if missing)"
    ))
...
plot_infections <- function(fit_summary,
  ...
  plot <- fit_summary[["latent"]] %>%
    ...
    geom_ribbon(aes(ymin = .lower, ymax = .upper, y = iota),
      fill = "#ffbf80",
      alpha = 0.3
    ) +
...
plot_R <- function(fit_summary,
                   ...
                   y_lim = NULL,
                   R_type = "R",
                   # can also be "R_epiestim" or "R_renewal"
  ...
  plot <- fit_summary[[R_type]] %>%
    ...
    geom_hline(yintercept = 1, linetype = "dashed") +
    {
      if (!is.null(truth) &&
        ("R" %in% names(truth))) {
        geom_line(data = truth, aes(x = date, y = R), color = "red")
      }
    } +
...
plot_delay <- function(fit_summary,
  ...
  plot <- fit_summary[["mean_delay"]] %>%
    ...
    {
      if (!is.null(truth) &&
        ("mean_delay" %in% names(truth))) {
        geom_line(
          data = truth,
          aes(x = date, y = mean_delay),
          color = "red"
        )
      }
    } +
    ...
    theme(legend.position = "none") +
    coord_cartesian(
      ylim = c(min_delay * 0.95, max_delay * 1.05),
      xlim = c(mindate + 1, maxdate)
    ) +
...
.getDateBreaks <- function(from, to, breaks_resolution = NULL) {
  if (is.null(breaks_resolution)) {
    ...
    date_breaks <-
      seq.Date(as.Date(to), as.Date(from), by = -breaks_resolution)
  }
  return(date_breaks)
}
...
plot_validate <-
  function(nowcasts,
           ...
           time_window,
           with_naive = F,
           plot_R_model = c("epiestim", "renewal"),
           ...) {
    if (missing(delay_plot)) {
      delay_plot <- unique(nowcasts$delay)
    }
    if (missing(plot_nowcast_date)) {
      plot_nowcast_date <- unique(nowcasts$nowcast_date)
    }
    ...
    if (with_naive) {
      rep_cases <- nowcasts %>%
        ...
        complete(
          ...
          nowcast_date = seq.Date(
            min(nowcast_date, na.rm = T),
            max(nowcast_date, na.rm = T),
            by = "1 day"
          )
        ) %>%
    ...
    if ("R" %in% names(nowcasts)) {
      input_R <- nowcasts %>%
        mutate(.width = 0.95) %>%
        filter(
          R_model %in% plot_R_model,
          delay %in% delay_plot,
          nowcast_date %in% plot_nowcast_date
        ) %>%
        select(date,
          delay,
          nowcast_date,
    ...
    if ("mean_delay" %in% names(nowcasts)) {
      input_mean_delay <- nowcasts %>%
        ...
        select(date,
          ...
          nowcast_date,
          mean_delay,
          .upper = mean_delay.upper,
          .lower = mean_delay.lower
        )
    } else {
    ...
    if (missing(time_window)) {
      time_window <-
        max(nowcasts$nowcast_date, na.rm = T) - min(nowcasts$date, na.rm = T)
    }
